...
# Parse .terraform.lock.hcl to get actual provider information
data "external" "parse_lock_file" {
  program = ["bash", "-c", <<-EOF
    lockfile=".terraform.lock.hcl"
    
    if [ ! -f "$lockfile" ]; then
      echo '{"error":"lock_file_not_found","provider_count":"0","message":"Run terraform init first"}'
      exit 0
    fi
    ...
    # Count total hashes (indicates platform support)
    hash_count=$(grep -c "^    \"h1:" "$lockfile" || echo "0")
    
    echo "{\"provider_count\":\"$provider_count\",\"lock_file_size_bytes\":\"$file_size\",\"total_hashes\":\"$hash_count\",\"providers\":\"$providers\"}"
  EOF
  ]
}
